...
class LyricTextAnimation: CALayer {
    ...
    func makeAnimationTool(_ videoSize: CGSize) -> AVVideoCompositionCoreAnimationTool {
...
extension LyricTextAnimation {
    ...
    func colorChangeAnimation(info: (Int, Int, String)) {
        ...
        let scale: CGFloat = self.frame.size.width / kScreenWidth
        ...
        var attrs: [NSAttributedString.Key : Any] = [
            .font: UIFont(name: "ArialRoundedMTBold", size: 30 * scale) as Any,
        ...
        attributedText.addAttributes(attrs, range: NSRange(location: 0, length: (text as NSString).length))
        
        let textLayer = CATextLayer()
        textLayer.string = attributedText
        textLayer.shouldRasterize = true
        textLayer.rasterizationScale = UIScreen.main.scale * scale
        textLayer.backgroundColor = UIColor.clear.cgColor
        textLayer.isWrapped = true
        textLayer.opacity = 0.0
        
        let maxWidth = self.frame.size.width - 10 * 2 * scale
        ...
        textLayer.frame = CGRect(x: 10.0 * scale, y: 100.0 * scale, width: stringSize.width + 5, height: stringSize.height + 20)
        textLayer.position = position
        
        attrs[.foregroundColor] = UIColor.green.cgColor
        attributedText.addAttributes(attrs, range: NSRange(location: 0, length: (text as NSString).length))
...
extension LyricTextAnimation {
    ...
    func slowShowAnimation(info: (Int, Int, String)) {
        ...
        let opacityAnimation = CAKeyframeAnimation(keyPath: "opacity")
        opacityAnimation.values = [0.0, 0.9, 1.0, 0.0]
        opacityAnimation.keyTimes = [0.0, 0.05, 0.95, 1.0]
        opacityAnimation.duration = Double(duration) / 1000.0
        ...
        for index in 1...text.count {
            ...
            attributedText.addAttributes(attrs, range: NSRange(location: 0, length: (subtext as NSString).length))
